...
- **답변 신뢰성 가드(`guard_node`)**: LLM이 자유 텍스트로 답을 끝내려 할 때 정규식+
  상태로 결정론적으로 검증 - 근거 없는 법령 인용, 건축물대장 조회 후 기록 누락,
  소방ᆞ간판 판정 누락, 수치 정정 누락 등을 잡아 재시도를 유도(프롬프트 지시만으로는
  못 막는 사례들을 실사용 중 발견해 도입)
- **Step1~9 체크리스트 기반 단계별 안내**: 각 단계(주소ᆞ건축물 현황 확인→신고ᆞ허가
  판단→건축사사무소 선정→…→오픈 준비)마다 실제 해야 할 일 체크리스트가 있고,
  챗봇은 매 턴 사용자가 지금 어디에 있는지(어떤 항목이 끝났고 안 끝났는지) 확인해
  다음 항목부터 자연스럽게 안내한다(2026-08-05 재설계 - 고정된 안내 순서를 텍스트
  마커로 강제하던 방식에서, 실제 체크리스트 상태를 매 턴 참고하는 방식으로 전환)
...
Bot: 기존 사무실을 카페로 바꾸시는 거면 용도변경 신고 대상입니다 [출처: 건축법 제19조].
     관할 강남구청 건축과에 접수하시면 되고, 서류ᆞ절차는 순서대로 안내해 드릴게요.
     먼저 건축사사무소를 통해 진행하실 건가요, 직접 진행하실 건가요?

You: 직접 하려고

Bot: 네, 건축사 설계 의무 대상은 아니라 직접 진행 가능합니다. 다음으로 필요 서류를
     안내해 드릴게요 - 용도변경 신고서, 임대차계약서, 평면도 등이 필요합니다...

...(챗봇이 매 턴 지금 단계의 체크리스트를 참고해 안 끝난 항목부터 이어서 안내 -
인허가 판정이 끝나면 공사 단계, 이어서 식품위생 영업신고ᆞ소방시설ᆞ간판ᆞ
사업자등록ᆞ위생교육까지 로드맵을 따라 순서대로/병렬로 계속 안내)
```
