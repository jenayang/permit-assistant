...
> 서울시 카페ᆞ음식점 등 소규모 창업의 전체 여정을 안내하는 AI 어시스턴트

건축 인허가(신축ᆞ증축ᆞ용도변경ᆞ대수선 등) 판정에서 시작해, 공사(착공신고ᆞ사용승인)ᆞ
식품위생 영업신고ᆞ소방시설ᆞ간판ᆞ사업자등록ᆞ위생교육ᆞ오픈 준비까지 창업 전 과정을
하나의 로드맵으로 안내합니다. LangGraph 기반 ReAct 에이전트가 법령/조례를 검색해
답하되, 허가/신고 여부처럼 법령상 객관적 기준으로 정해지는 판정은 LLM이 아니라
코드가 결정론적으로 계산합니다.
...
- **LLM**: Google Gemini(`gemini-3.1-flash-lite`, 기본) → Cerebras(할당량 소진 시 자동 폴백)
...
- **에이전트**: LangGraph(ReAct 패턴) + `SqliteSaver` 체크포인터(대화 상태 영구 저장)
- **부지 자동조회**: VWorld API(용도지역ᆞ지오코딩/역지오코딩), 공공데이터포털 건축HUB(건축물대장)
- **API / UI**: FastAPI + 정적 HTML/JS 채팅 UI
- **테스트**: pytest (파이썬 로직) + Node.js subprocess(프론트 로드맵 잠금 로직 검증)
...
│   ├── agent.py             # LangGraph 에이전트(ReAct 루프ᆞguard_nodeᆞSystemPrompt)
