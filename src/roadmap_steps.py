...
def _resolve_pre_diagnosis(state: dict) -> ResolvedSubstep:
    ...
    return ResolvedSubstep(
        ...
        content_guide=(
            ...
            "왜 지금 이 단계를 하는지 한 줄로 먼저 짚은 뒤, 다음 항목 중 실제 해당하는 "
            "것만 1~2줄+근거와 함께 안내하세요(각 항목의 근거 법령을 아래에 적어뒀으니 "
            "search_regulations/search_by_term으로 그 법령을 대상으로 검색하세요 - "
            "일반 키워드로만 검색하면 엉뚱한 조문이 걸릴 수 있습니다):\n"
            "- 주차대수: 서울시 주차장 설치 및 관리 조례(부설주차장 설치기준 별표) - "
            "업종ᆞ시설면적별 기준 대수\n"
            "- 정화조: 하수도법 제34ᆞ35조 - 오수처리시설ᆞ정화조 설치의무ᆞ처리대상인원\n"
            "- 소방시설: 소방시설 설치 및 관리에 관한 법률 시행령(별표4) - 연면적ᆞ업종별 "
            "소화기구ᆞ옥내소화전 등 설치대상 여부(다중이용업이면 다중이용업소의 안전관리에 "
            "관한 특별법도 함께 확인)\n"
            "- 장애인 편의시설: 장애인ᆞ노인ᆞ임산부 등의 편의증진 보장에 관한 법률"
            "(장애인등편의법) - 시설 규모ᆞ용도에 따른 설치대상 여부\n"
...
def _resolve_documents_prepared(state: dict) -> ResolvedSubstep:
    ...
    return ResolvedSubstep(
        ...
        content_guide=(
            ...
            "제출할 것인지 위한 서류\"를 실제 작성 주체 중심으로 안내하세요. 필요 서류의 "
            "기본 목록ᆞ첨부서류는 건축법 시행규칙 제6조(건축허가 등의 신청)ᆞ제12조의2"
            "(용도변경) - search_regulations/search_by_term으로 이 조문을 대상으로 검색해 "
            "실제 항목을 확인하세요(이미 판정 직후 자동 검색된 결과가 도구 응답에 있으면 "
            "그대로 재사용). 법적 명의자는 항상 건축주(신청인)이지만, 실제로 누가 "
            "작성하는지는 직접/대행 여부에 따라 다릅니다 - \"건축사사무소가 대행해준다\"는 "
            "법적 요건이 아니라 대행을 선택했을 때만 해당하는 실무 관행입니다(법조문은 "
            "\"신청서를 제출하는 자\"만 규정하고 작성 주체는 명시하지 않음 - 임의로 \"보통 "
            "대행해준다\"고 단정하지 마세요). 필요 서류를 하위 항목으로 나열하되, 단순 "
            "나열이 아니라 각 서류 옆에 실제 작성 주체를 함께 표시하세요. 다른 단계와 같은 "
            f"마크다운 서식(굵게 **소제목**, 목록은 - 불릿)을 그대로 쓰세요. {example}\n"
