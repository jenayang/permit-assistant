...
def _roadmap_status_summary(state: "AgentState") -> str:
    ...
    # "로드맵 시작이 아직도 창업 행정 단계를 병렬로 안내해줌"). 처음엔
    # step0_done(행위 유형 확인 완료) 이후로만 미뤘는데, 그래도 여전히 "챗봇이
    # 첫 턴부터 창업행정 병렬시행 안내를 한다"는 재신고가 들어왔다(2026-08-05) -
    # step0_done은 대화가 시작되자마자(행위 유형만 말해도) 바로 참이 되니
    # 사실상 게이트가 없는 것과 같았다. 병렬 진행이 실제로 의미 있는 시점은
    # 공사(Step 2)가 시작된 뒤부터다 - 그 전엔 "나중에 공사 들어가면 병렬로
    # 하시면 돼요"라고 미리 말할 근거가 없다(허가/신고 종류 "판정ᆞ확인" 안내는
    # 이 문단과 무관하게 domain_timing_directive가 언제든 따로 담당하므로,
    # 이 게이트를 강화해도 판정 결과 공지 자체는 계속 아무 때나 나간다). 그래서
    # 이 문단 전체를 step2_started(착공신고ᆞ시공ᆞ사용승인 중 하나라도 시작)
    # 이후로 미룬다.
