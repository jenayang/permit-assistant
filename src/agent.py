...
[Step 1-4] 절차ᆞ사전검토ᆞ서류ᆞ신청 내용을 먼저 설명하지 마세요** - 아직 판정도 안 났는데
...
_COLLECT_PROGRESS = """사용자가 사전 진단 항목 확인ᆞ필요 서류 준비ᆞ신청서 접수ᆞ착공신고ᆞ시공ᆞ
사용승인ᆞ사업자등록ᆞ위생교육ᆞ인테리어ᆞ장비설치ᆞ직원등록ᆞ영업개시 중 **실제로
...
하나요?" 같은 계획ᆞ질문 표현은 완료가 아니고, [Step 1-2]/[Step 1-3]에서
사전진단ᆞ서류 항목을 "안내"한 것도 완료가 아닙니다(pre_diagnosis_checked/
documents_prepared는 사용자가 "사전 진단 확인했어요"/"서류 다 준비했어요"처럼
직접 확인해줬을 때만, application_submitted는 [Step 1-4] 안내와 무관하게
사용자가 "접수했어요"/"신청서 냈어요"처럼 실제 접수를 직접 말해줬을 때만).
이 항목들은 실물 세계에서 벌어지는 일이라 오직 사용자가 직접 완료를 말해준
경우에만 기록하세요.
...
질문 등): 아래 4단계로 나눠 순서대로 안내하세요(2026-08-03 순서 재정렬 -
실제 업무 순서인 "판정 → 사전 검토 → 서류 준비 → 신청/접수"를 반영. 예전엔
"신청서 제출"을 사전진단보다 먼저 안내했는데, 실제로는 사전진단(주차ᆞ정화조ᆞ
소방 등) 결과가 설계도서에 반영돼야 하니 순서가 거꾸로였다). 각 단계는
핵심만 간결하게 - 이전 단계/턴에서 말한 내용을 다시 설명하지 마세요.
**각 단계 안내를 시작할 때 "왜 지금 이 단계를 하는지" 한 줄을 먼저 붙이세요**
(아래 각 단계별 문구 참고) - 체크박스만 채워주는 도구가 아니라 이유를
설명해주는 것이 이 어시스턴트의 핵심 역할입니다. 각 단계 끝에 다음 단계를
...
제안 문장은 "다음으로 사전 검토 내용을 안내해 드릴까요?"처럼 괄호 헤더
없이 평문으로 쓰세요.
**판정이 확정되는 순간 시스템이 이미 관련 법령을 한 번 자동 검색해서
도구 응답으로 넣어뒀습니다** - [Step 1-3: 서류 준비]의 첨부 서류를 쓸 때 그
...
(required_documents, related_agencies). [Step 1-2: 사전 검토]도 마찬가지로, 안내한 항목들을
...
- [Step 1-2: 사전 검토] "설계 전에 법적 기준(주차ᆞ소방 등)을 먼저 확인하는
  단계입니다 - 여기서 문제가 나오면 설계를 수정해야 하므로 서류 준비 전에
  끝내는 게 좋습니다"처럼 왜 지금 이 단계를 하는지 한 줄로 먼저 짚은 뒤,
  주차대수ᆞ정화조 용량ᆞ소방시설ᆞ장애인 편의시설ᆞ위생요구사항 중 실제
  해당하는 것만 1~2줄+근거와 함께 안내하세요 -
  record_permit_synthesis(pre_diagnosis_items=[...]).
- [Step 1-3: 서류 준비] "설계도서 작성은 건축사사무소와 상담해서 준비하는
  과정이라 보통 몇 주가 걸립니다"처럼 시간이 걸리는 단계라는 감각을 먼저
  준 뒤, **"어디에 무엇을 제출할 것인지 위한 서류"를 준비주체 중심으로**
  안내하세요. **필요 서류를 하위 항목으로** 나열하되, 단순 나열이 아니라
  **각 서류 옆에 준비 주체를 함께 표시**하세요("이건 내가, 이건 사무소ᆞ
  업체가"가 드러나게). 다른 단계와 같은 마크다운 서식(굵게 `**소제목**`,
  목록은 `-` 불릿)을 그대로 쓰세요 - 이 단계만 다른 서식을 쓸 이유가 없습니다.
  예시 구조(문구는 상황에 맞게):
  **[본인 준비]**
  - 신고ᆞ신청서(별지 서식)
  - 대지 소유ᆞ사용권원 서류(등기부등본ᆞ임대차계약서 등)
  **[설계자 준비]**
  - 평면도ᆞ배치도ᆞ내화ᆞ방화ᆞ피난ᆞ설비 도서 등 설계도서 - 건축사 의무 대상이면
    "건축사사무소", 아니면 "인테리어 업체 또는 본인"으로 표기(도구 응답의 건축사
    의무 여부를 따름, 당신이 판단하지 말 것)
  **[관공서 자체확인]**
  - 용도변경 시 변경 전 평면도는 관공서가 건축물대장으로 직접 확인하므로
    본인이 준비할 필요 없음
  record_permit_synthesis(required_documents=[...])
- [Step 1-4: 신청ᆞ접수] "서류가 준비되면 실제로 접수해 허가/신고 수리를
  받는 단계입니다"처럼 짚은 뒤, 관할 시ᆞ군ᆞ구청에 신청ᆞ신고서(별지 서식)와
  위 서류를 제출(전자문서ᆞ온라인 접수 가능)하고, 관계 법령상 협의가 필요하면
  협의기관 검토를 거쳐 허가/신고가 수리된다는 흐름을 한두 줄로 안내하세요
  (협의기관이 확인되면 record_permit_synthesis(related_agencies=[...])).
  안내를 마치면서, 예상 소요 기간(허가 15~20일ᆞ신고 3~5일ᆞ기재변경 3~7일
  등 통상적으로 알려진 수준)을 별도 헤더 없이 자연스러운 한 문장으로
  덧붙이세요 - 로드맵 체크리스트엔 대상판정ᆞ사전검토ᆞ서류준비ᆞ신청접수
  4개만 있어 소요 기간에 별도 [Step 1-N] 게이트를 두지 않습니다.
  search_regulations로 실제 처리기한을 확인했으면 그 값을 우선 쓰고, 못
...
def permit_phase_directive(state: AgentState) -> str | None:
    ...
    if disclosed == 2 and not task_progress.get("pre_diagnosis_checked"):
        return (
            "[진행 상태] Step 1-2(사전 검토) 안내까지 끝났습니다. 이번 턴에는 Step 1-3(서류 "
            "준비)로 넘어가지 말고 \"사전 검토 항목은 확인해 보셨어요?\"처럼 완료 여부부터 "
    ...
    if disclosed == 3 and not task_progress.get("documents_prepared"):
        return (
            "[진행 상태] Step 1-3(서류 준비) 안내까지 끝났습니다. 이번 턴에는 Step 1-4(신청ᆞ"
            "접수)로 넘어가지 말고 \"서류 준비는 다 되셨어요?\"처럼 완료 여부부터 확인하세요. "
...
# 게이트 수를 1:1로 맞췄었다.
# 2026-08-03: 다시 4단계로 확장하되 이번엔 내용 자체가 다르다 - 실제 업무
# 순서(대상판정→사전검토→서류준비→신청접수)를 반영해 "신청서 제출"에
# 뭉쳐 있던 서류 준비(몇 주 걸림)와 접수(몇 분짜리 행위)를 분리하고,
# 사전진단을 서류 준비보다 앞으로 옮겼다(사전진단 결과가 설계도서에
# 반영돼야 하므로). 로드맵 체크리스트도 Step1 항목을 4개로 함께 늘렸다
# (project_status.py/index.html) - 3단계 축소 때와 마찬가지로 대화 게이트
# 수와 체크리스트 항목 수를 1:1로 유지한다.
